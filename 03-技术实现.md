...
- **主从结构** ：**主 OpenClaw（发财）** 对接飞书（独立机器人），负责接收指令、派单与调度；**聚宝、班工、美含、如一** 为 **Docker 上运行的子 OpenClaw 实例**；**执远** 因需操作本地浏览器（Playwright/Chrome），部署在**宿主机**而非 Docker。各子实例在飞书项目群内响应与汇报，由主 OpenClaw（发财）按工单路由到对应子实例执行。
...
- **流量** ：日常 人 ⇄ 发财（点对点）；有项目时 人/群消息 → **发财** 统一判断 → 派单到 **子 OpenClaw**（Docker 或宿主机执远）→ 子实例在**项目群**内汇报、写**飞书文档**，回写 DB。
...
### 3.4 长期记忆与向量检索 (memory-lancedb-pro) ⭐ 推荐启用
